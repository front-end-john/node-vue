...
UserModel.prototype.queryUser = function (data, callback) {
   var keyword = data.keyword; // 获取查询的字段
   var _filter = { //  $options: '$i' 忽略大小写
      $or: [  // 多字段同时匹配
   ...
   var count = 0;
   this.User2.countDocuments(_filter, function (err, doc) { // 查询总条数（用于分页）
      if (err) {
         console.log(err);
      } else {
         count = doc;
   ...
   this.User2.find(_filter).limit(10) // 最多显示10条
       .sort({'_id': -1}) // 倒序
